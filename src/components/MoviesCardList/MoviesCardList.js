...
export default function MoviesCardList({ isSaved, movies,savedMovies, isProcessed,  isRequestSuccess, onAddMovie, moviesListSize, onLike, onUnlike}) {
  ...
  return (
    <section className={`movies-card-list ${isProcessed ? "movies-card-list_none" : ""}`} >
      ...
      <div>{!isProcessed &&  ((!movies.length ? ( isRequestSuccess ? "Ничего не найдено по Вашему завпросу. " : "Во время запроса произошла ошибка. Возможно, проблема с соединением или сервер недоступен. Подождите немного и попробуйте ещё раз. ") : "") + (( !movies.length) ? "Ввeдите запрос(не пустую строку) в поисковую строку и нажмите на кнопу \"Найти\" для отображения фильмов. Для отображения короткометражных фильмов устновите значение переключателя \"Короткометражки\" перед отправкой запроса.": ""))}</div>
      {( (<button
